...
#
# Prereq: ~/.pgpass has entries for localhost:5434:madonnahist:<role>:...
#         for madonnahist_owner, madonnahist_app, madonnahist_worker.

set -uo pipefail
unset PGPASSWORD  # don't let an env-set PGPASSWORD override .pgpass

DB="-h localhost -p 5434 -d madonnahist"
OWNER="$DB -U madonnahist_owner"
APP="$DB -U madonnahist_app"
WORKER="$DB -U madonnahist_worker"
...
# ─────────────────────────────────────────────────────────────────
psql $OWNER -v ON_ERROR_STOP=1 -q <<'SQL'
...
# ─────────────────────────────────────────────────────────────────
out=$(psql $WORKER -v ON_ERROR_STOP=1 -tAc \
...
aux=$(psql $OWNER -tAc "SELECT search_aux_text FROM calendar_days WHERE id=1;")
...
# ─────────────────────────────────────────────────────────────────
psql $WORKER -v ON_ERROR_STOP=1 -q -c \
...
status=$(psql $OWNER -tAc "SELECT correction_status FROM calendar_days WHERE id=1;")
note=$(psql $OWNER -tAc "SELECT review_note FROM calendar_days WHERE id=1;")
...
ocr_id=$(psql $OWNER -tAc "SELECT latest_ocr_run_id FROM calendar_days WHERE id=1;")
conf=$(psql $OWNER -tAc "SELECT latest_confidence_score FROM calendar_days WHERE id=1;")
...
psql $OWNER -q -c "UPDATE calendar_days SET correction_status='pending', review_note=NULL WHERE id=1;" > /dev/null
...
# ─────────────────────────────────────────────────────────────────
out=$(psql $APP -v ON_ERROR_STOP=1 -tAc \
...
status=$(psql $OWNER -tAc "SELECT correction_status FROM calendar_days WHERE id=1;")
text=$(psql $OWNER -tAc "SELECT corrected_text FROM calendar_days WHERE id=1;")
by=$(psql $OWNER -tAc "SELECT corrected_by FROM calendar_days WHERE id=1;")
...
text=$(psql $OWNER -tAc "SELECT corrected_text FROM calendar_days WHERE id=1;")
by=$(psql $OWNER -tAc "SELECT corrected_by FROM calendar_days WHERE id=1;")
at=$(psql $OWNER -tAc "SELECT corrected_at FROM calendar_days WHERE id=1;")
