#!/usr/bin/env bash
# Deploy madonnahist to the shared DigitalOcean droplet.
#
# Flow:
...
#      c. npm run build
#      d. backend/db/migrate_pg.sh      (connects as madonnahist_owner)
#      e. if deploy/nginx.conf checksum changed → copy + nginx -t + reload
#      f. pm2 restart madonnahist --update-env
#      g. health-check: db==ok required; spaces==not_configured tolerated
#
...
# nginx: only reload if config or origin cert has changed.
...
for i in 1 2 3 4 5 6; do
  ...
  if [[ -n "${body}" ]]; then
    ...
    spaces=$(printf '%s' "${body}" | sed -n 's/.*"spaces":"\([^"]*\)".*/\1/p')
    if [[ "${db}" == "ok" ]] && [[ "${spaces}" != "error" ]]; then
      echo "==> health OK (db=${db}, spaces=${spaces})"
      exit 0
    fi
  fi
  sleep 5
