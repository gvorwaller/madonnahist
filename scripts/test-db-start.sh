...
SUPERUSER="${MADONNAHIST_TEST_SUPERUSER:-$(id -un)}"

mkdir -p "$REPO_ROOT/.local"

if [[ ! -f "$MADONNAHIST_TEST_PGDATA/PG_VERSION" ]]; then
  echo "Initializing local test Postgres cluster at $MADONNAHIST_TEST_PGDATA"
  "$INITDB_BIN" -D "$MADONNAHIST_TEST_PGDATA" --auth-local=trust --auth-host=scram-sha-256 --no-instructions >/dev/null
fi

if "$PG_CTL_BIN" -D "$MADONNAHIST_TEST_PGDATA" status >/dev/null 2>&1; then
  echo "Local test Postgres already running."
else
  echo "Starting local test Postgres on $PGHOST:$PGPORT"
  "$PG_CTL_BIN" -D "$MADONNAHIST_TEST_PGDATA" -l "$MADONNAHIST_TEST_PGLOG" -o "-p $PGPORT -h $PGHOST" -w start
fi
...
env -u PGHOST -u PGUSER -u PGPASSWORD "$PSQL_BIN" -p "$PGPORT" -d postgres -U "$SUPERUSER" -v ON_ERROR_STOP=1 \
  ...
  -v app_user="$PGUSER" \
  -v app_pw="$PGPASSWORD" \
  -v worker_user="$WORKER_PGUSER" \
  -v worker_pw="$WORKER_PGPASSWORD" \
...
SELECT format('CREATE ROLE %I LOGIN PASSWORD %L', :'worker_user', :'worker_pw')
WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = :'worker_user') \gexec
SELECT format('ALTER ROLE %I WITH LOGIN PASSWORD %L', :'worker_user', :'worker_pw') \gexec
