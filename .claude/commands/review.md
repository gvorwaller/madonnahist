...
>
> **Repository context:** Node/Express backend with PostgreSQL (`pg`) and a React + Zustand frontend. This is the madonnahist family-calendar archive app. Focus on PostgreSQL type-boundary issues (JSONB-as-object, NUMERIC-as-string, TIMESTAMPTZ/UTC handling), async/await on database queries, OCR pipeline correctness (idempotent re-runs must never overwrite human-corrected text), DO Spaces integration (image paths are object keys, not filesystem paths), and React-specific concerns (effect dependencies, stale closures, Zustand store subscriptions). Read the project's `cs.md` and `CLAUDE.md` for additional context, especially the data integrity SACRED RULES.
>
